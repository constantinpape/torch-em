...
class TestRawDataset(unittest.TestCase):
    ...
    def test_dataset(self):
        ...
        ds = RawDataset(self.path, self.raw_key, patch_shape=patch_shape)
    ...
    def test_dataset_2d(self):
        from torch_em.data import RawDataset
        patch_shape = (1, 32, 32)
        ds = RawDataset(self.path, self.raw_key, patch_shape=patch_shape, ndim=2)
        self.assertEqual(ds.raw.shape, self.shape)
    ...
    def test_roi(self):
        ...
        ds = RawDataset(self.path, self.raw_key, patch_shape=patch_shape, roi=roi)
        roi_shape = 3 * (64,)
    ...
    def test_with_channels(self):
        from torch_em.data import RawDataset
        patch_shape = (32, 32, 32)
        ds = RawDataset(self.path, self.channel_key, patch_shape=patch_shape, ndim=3, with_channels=True)
        self.assertEqual(ds._ndim, 3)
