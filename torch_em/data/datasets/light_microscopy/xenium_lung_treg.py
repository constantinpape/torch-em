"""This dataset contains cell and nucleus segmentation masks for whole-slide fluorescence
images of mouse lung tissue, from the BioImage Archive study S-BIAD3880.
...
Analysis (XOA) pipeline produced. Channel 0 always holds the DAPI nuclear stain. The other
channels hold the boundary and interior stains of the multi-tissue stain mix, which XOA uses
to draw the cell outlines.

NOTE: The masks are the output of XOA, not a manual annotation. XOA segments the nuclei on
DAPI, then grows each cell from its nucleus using the boundary and interior stains. Where no
stain supports a boundary it falls back to expanding the nucleus by 5 micrometer, so a share
of the cells are dilated nuclei rather than observed outlines. The 'nuc_expansion_fraction'
attribute of each h5 file reports that share. Treat this data as a weak label source for
pretraining, not as a benchmark.
...
The data is hosted at https://www.ebi.ac.uk/biostudies/bioimages/studies/S-BIAD3880 under the
CC0 1.0 license. Please cite the corresponding study page if you use this data in your research.
"""
...
URLS = {
    "ctrl69": f"{BASE_URL}/ctrl69_xenium_output.zip",
    "ctrl70": f"{BASE_URL}/ctrl70_xenium_output.zip",
    "ntcu06": f"{BASE_URL}/ntcu06_xenium_output.zip",
    "ntcu36": f"{BASE_URL}/ntcu36_xenium_output.zip",
    "ntcu46": f"{BASE_URL}/ntcu46_xenium_output.zip",
    "pi42": f"{BASE_URL}/pi42_xenium_output.zip",
    "pi51": f"{BASE_URL}/pi51_xenium_output.zip",
    "pi57": f"{BASE_URL}/pi57_xenium_output.zip",
}

# The treatment group of every sample, for the h5 attributes.
TREATMENT = {
    "ctrl69": "control",
    "ctrl70": "control",
    "ntcu06": "ntcu",
...
def _remote_zip_index(url: str) -> Dict[str, Dict[str, int]]:
    ...
    can skip the transcripts, the per-cycle QC images, and the expression matrix, which
...
def _find_root(index: Dict[str, Dict[str, int]], sample: str) -> str:
    """Find the single top-level output folder that every member of the bundle sits under."""
    roots = {name.split("/")[0] for name in index if "/" in name}
    if len(roots) != 1:
        raise RuntimeError(f"Expected exactly one output folder in the bundle for '{sample}', found {roots}.")
    return next(iter(roots)) + "/"


def _download_member(url: str, name: str, entry: Dict[str, int], output_path: str) -> None:
    """Read one member out of a remote zip and write it, checking its crc32."""
...
def _download_bundle(url: str, bundle_dir: str, sample: str, download: bool, with_stains: bool) -> str:
    ...
    root = _find_root(index, sample)
    channels = natsorted(
        n for n in index if n.startswith(f"{root}morphology_focus/") and n.endswith(".ome.tif")
    )
    if not channels:
    ...
    names = [f"{root}experiment.xenium", f"{root}cells.zarr.zip"] + channels
    ...
    for name in names:
        relative_name = name[len(root):]
        _download_member(url, name, index[name], os.path.join(temporary_dir, relative_name))
...
def _channel_paths(bundle_dir: str) -> List[str]:
    """Return the morphology channels, DAPI first."""
...
def _copy_blockwise(source, target, channel: Optional[int] = None) -> int:
    ...
    The target must be indexed in one step, because indexing an h5 dataset twice writes into
    a copy of the data rather than into the file.
...
def _create_h5(bundle_dir: str, output_path: str, sample: str, with_stains: bool) -> str:
    ...
    with h5py.File(temporary_path, "w") as f:
        ...
        f.attrs["treatment_group"] = TREATMENT[sample]
        f.attrs["axes"] = "yx"
...
def get_xenium_lung_treg_data(
    ...
    This function reads the central directory of the remote zip that BioImage Archive
    hosts for the sample, then reads only the members it needs (the manifest, the cell and
    nucleus masks, and the morphology channels). It does not fetch the transcripts, the
    per-cycle QC images, or the expression matrix, which cuts the download by a large margin.
...
def get_xenium_lung_treg_paths(
    path: Union[os.PathLike, str],
    ...
    return [get_xenium_lung_treg_data(path, name, download, with_stains) for name in samples]


def get_xenium_lung_treg_dataset(
    path: Union[os.PathLike, str],
    ...
    **kwargs,
    ...
    volume_paths = get_xenium_lung_treg_paths(path, sample, download, with_stains=raw_channel == "stack")
...
def get_xenium_lung_treg_loader(
    path: Union[os.PathLike, str],
    batch_size: int,
    patch_shape: Tuple[int, int],
    sample: Optional[Union[str, Sequence[str]]] = None,
    ...
    **kwargs,
    ...
    dataset = get_xenium_lung_treg_dataset(
        path=path,
        patch_shape=patch_shape,
        sample=sample,
