"""The CPTAC-UCEC-Tumor-Annotations dataset contains annotations for endometrial (uterine corpus) carcinoma
and its metastatic lesions in CT.

The dataset consists of contrast-enhanced CT or MR series of endometrial cancer patients, with expert
...
The dataset is located at https://doi.org/10.7937/89M3-KQ43 and is distributed under the
CC BY 4.0 license.
This dataset is from the CPTAC-UCEC collection on The Cancer Imaging Archive; please cite the DOI
...
def _get_series_metadata(path, download):
    ...
    metadata_path = os.path.join(path, "cptac_ucec_tumor_series.json")
...
def _preprocess_cptac_ucec_tumor(dicom_dir, series_metadata, preprocessed_dir):
    import h5py
    import pydicom
    ...
    for series in tqdm(rtstruct_series, desc="Preprocess CPTAC-UCEC-Tumor-Annotations"):
...
def get_cptac_ucec_tumor_data(path: Union[os.PathLike, str], download: bool = False) -> str:
    ...
    if download:  # The RTSTRUCT series are downloaded first, so the image series they reference can be found.
        util.download_tcia_series(
            rtstruct_uids, dst=dicom_dir, csv_filename=os.path.join(path, "cptac_ucec_tumor_rtstruct")
        )
        ...
        util.download_tcia_series(
            sorted(image_uids), dst=dicom_dir, csv_filename=os.path.join(path, "cptac_ucec_tumor_image")
        )
    elif not glob(os.path.join(dicom_dir, "*", "*.dcm")):
        raise RuntimeError(f"Cannot find the data at {path}, but download was set to False.")

    _preprocess_cptac_ucec_tumor(dicom_dir, series_metadata, preprocessed_dir)
...
def get_cptac_ucec_tumor_paths(path: Union[os.PathLike, str], download: bool = False) -> List[str]:
    ...
    """
    preprocessed_dir = get_cptac_ucec_tumor_data(path, download)
...
def get_cptac_ucec_tumor_dataset(
    ...
    **kwargs
    ...
    """
    volume_paths = get_cptac_ucec_tumor_paths(path, download)
...
def get_cptac_ucec_tumor_loader(
    path: Union[os.PathLike, str],
    batch_size: int,
    patch_shape: Tuple[int, ...],
    resize_inputs: bool = False,
    ...
    **kwargs
    ...
    dataset = get_cptac_ucec_tumor_dataset(path, patch_shape, resize_inputs, download, **ds_kwargs)
