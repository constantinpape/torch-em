"""The LES-AV dataset contains annotations for artery-vein segmentation in fundus images,
paired with glaucoma diagnosis information.
...
This dataset is located at https://figshare.com/articles/dataset/LES-AV_dataset/11857698.
The dataset is from the publication https://doi.org/10.1007/978-3-030-00934-2_8.
...
def _process_labels(data_dir):
    label_paths = glob(os.path.join(data_dir, "arteries-and-veins", "*.png"))
...
def get_les_av_data(path: Union[os.PathLike, str], download: bool = False) -> str:
    ...
    """
    data_dir = os.path.join(path, "LES-AV")
    if os.path.exists(data_dir):
        return data_dir

    os.makedirs(path, exist_ok=True)

    zip_path = os.path.join(path, "LES-AV.zip")
...
def get_les_av_paths(path: Union[os.PathLike, str], download: bool = False) -> Tuple[List[str], List[str]]:
    ...
    """
    data_dir = get_les_av_data(path=path, download=download)

    image_paths = sorted(glob(os.path.join(data_dir, "images", "*.png")))
    label_paths = sorted(glob(os.path.join(data_dir, "arteries-and-veins", "*.tif")))

    return image_paths, label_paths


def get_les_av_dataset(
    ...
    **kwargs
    ...
    """
    image_paths, label_paths = get_les_av_paths(path=path, download=download)
...
def get_les_av_loader(
    ...
    **kwargs
    ...
    dataset = get_les_av_dataset(path, patch_shape, resize_inputs, download, **ds_kwargs)
