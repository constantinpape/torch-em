"""The OpticLobe dataset contains a FIB-SEM volume of the Drosophila right optic lobe
with dense neuron instance segmentation, covering the right optic lobe subset of the
MaleCNS sample (about 50,000 neurons) at 8 nm isotropic resolution.

The EM volume is at gs://flyem-optic-lobe/grayscale-clahe-jpeg and the segmentation is at
gs://flyem-optic-lobe/v1.1/segmentation.

This dataset is from the publication https://doi.org/10.1038/s41586-025-08746-0.
Please cite it if you use this dataset in your research.

The dataset is publicly available at https://www.janelia.org/project-team/flyem/optic-lobe
under the CC-BY license.
Requires cloud-volume: pip install cloud-volume.

NOTE (on data size): the full volume is (36864, 55296, 45056) voxels at 8 nm isotropic
...
def get_optic_lobe_data(
    ...
    print(f"Streaming OpticLobe EM + segmentation for bbox {bounding_box} ...")
...
def get_optic_lobe_paths(
    ...
    return [get_optic_lobe_data(path, bbox, download) for bbox in bounding_boxes]


def get_optic_lobe_dataset(
    path: Union[os.PathLike, str],
    ...
    **kwargs,
    ...
    paths = get_optic_lobe_paths(path, bounding_boxes, download)
...
def get_optic_lobe_loader(
    ...
    **kwargs,
    ...
    dataset = get_optic_lobe_dataset(
