...
def main(args):
    ...
    model = common.get_unetr_model(
        ...
        sam_initialization=args.do_sam_ini, output_channels=2
    )
    model.to(device)
    ...
    if args.train:
        print("2d UNETR training (with boundaries) on LiveCELL...")

        # get the desired livecell loaders for training
        train_loader, val_loader = common.get_my_livecell_loaders(
            args.input, patch_shape, args.cell_type, with_boundary=True,
    ...
    if args.predict:
        print("2d UNETR inference (with boundaries) on LiveCELL...")
    ...
    if args.evaluate:
        print("2d UNETR evaluation (with boundaries) on LiveCELL...")
        csv_save_dir = os.path.join("results", dir_structure)
        os.makedirs(csv_save_dir, exist_ok=True)

        common.do_unetr_evaluation(
            input_path=args.input, root_save_dir=root_save_dir, csv_save_dir=csv_save_dir
        )
