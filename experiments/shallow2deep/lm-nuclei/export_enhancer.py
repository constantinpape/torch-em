...
def _get_name_and_description(is3d):
    name = "EnhancerNucleiLM3D" if is3d else "EnhancerNucleiLM2D"
    description = "Prediction enhancer for segmenting nuclei in light microscopy."
    return name, description


def _get_doc(ckpt, name, is3d):
    training_summary = get_training_summary(ckpt, to_md=True, lr=1.0e-4)
    doc = f"""#Prediction Enhancer for Nucleus segmentation in LM
...
The network was trained on light microscopy data of nuclei
...
def create_input_2d():
    input_path = "/scratch/pape/s2d-lm-nuclei/mouse-embryo/Nuclei/train/E4_Ab1.h5"
    with open_file(input_path, "r") as f:
        data = f["raw"][-1, :512, :512]
    rf_path = "/scratch/pape/s2d-lm-nuclei/rfs2d/mouse-embryo/rf_0049.pkl"
...
def create_input_3d():
    ...
    rf_path = "/scratch/pape/s2d-lm-nuclei/rfs3d/mouse-embryo/rf_0049.pkl"
    filter_config = _get_filters(2, None)
...
def export_enhancer(is3d):
    ...
    checkpoint = "./checkpoints/s2d-lm-nuclei-mouse-embryo_root-3d" if is3d else\
        "./checkpoints/s2d-lm-nuclei-mouse-embryo_root-2d"
