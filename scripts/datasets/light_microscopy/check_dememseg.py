...
from torch_em.data.datasets import get_dememseg_loader
from torch_em.util.debug import check_loader

sys.path.append("..")


def check_dememseg(save_path=None):
    from util import ROOT

    path = os.path.join(ROOT, "dememseg")
    for split in ("train", "val", "test"):
        loader = get_dememseg_loader(
            path=path,
            batch_size=1,
            patch_shape=(200, 200),
            split=split,
