...
from torch_em.data.datasets import get_mrose_bacteria_loader
from torch_em.util.debug import check_loader

sys.path.append("..")


def check_mrose_bacteria(save_path=None):
    from util import ROOT

    path = os.path.join(ROOT, "mrose_bacteria")
    for split in ("train", "val", "test"):
        for label_choice in ("instances", "semantic"):
            loader = get_mrose_bacteria_loader(
                path=path,
                batch_size=1,
                patch_shape=(512, 512),
                split=split,
...
def main():
    ...
    check_mrose_bacteria(args.save_path)
