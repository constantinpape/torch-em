...
from torch_em.data.datasets.light_microscopy.xenium_lung_treg import URLS, get_xenium_lung_treg_loader

sys.path.append("..")


def check_xenium_lung_treg(samples=None, save_path=None):
    from util import ROOT

    path = os.path.join(ROOT, "xenium_lung_treg")
    for sample in samples or list(URLS):
        for label_channel in ("nuclei", "cells"):
            loader = get_xenium_lung_treg_loader(
                path=path,
                batch_size=1,
                patch_shape=(512, 512),
                sample=sample,
...
def main():
    ...
    check_xenium_lung_treg(args.sample, args.save_path)
