...
from torch_em.data.datasets import get_alfi_loader
from torch_em.util.debug import check_loader

sys.path.append("..")


def check_alfi(save_path=None):
    from util import ROOT

    path = os.path.join(ROOT, "alfi")
    for split in ("train", "val", "test"):
        for label_choice in ("instances", "semantic"):
            loader = get_alfi_loader(
                path=path,
                batch_size=1,
                patch_shape=(512, 512),
                split=split,
