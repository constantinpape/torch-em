...
from torch_em.data.datasets.electron_microscopy import get_openorganelle_nucleus_loader
from torch_em.data.datasets.electron_microscopy.openorganelle_nucleus import DATASETS

sys.path.append("..")


def check_openorganelle_nucleus(dataset_names, patch_shape, n_samples):
    from util import ROOT

    loader = get_openorganelle_nucleus_loader(
        path=os.path.join(ROOT, "openorganelle_nucleus"),
...
def main():
    ...
    check_openorganelle_nucleus(args.dataset_names, tuple(args.patch_shape), args.n_samples)
